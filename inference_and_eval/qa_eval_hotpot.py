        ...
        for sample_id in tqdm(unique_sample_ids):
            try:
                ...
                if isinstance(ans, int):
                    ans = str(ans)
                with jsonlines.open(
                    f"qa_logs/{args.triplets_db_name}_{args.model_name.replace('/', '_')}_structured_{args.structured_inference}_multi_step_{args.multi_step_qa}_use_qualifiers_{args.use_qualifiers}_use_filtered_triplets_{args.use_filtered_triplets}_hotpot_test_run_{str(args.run_number)}.jsonl",
                    "a",
                ) as f:
